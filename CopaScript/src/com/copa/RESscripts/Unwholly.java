...
public class Unwholly extends FlightSearchPageObjects {
	...
	public void Unwholly(WebDriver driver, BFrameworkQueryObjects queryObjects) throws Exception {
		...
		try {
			...
			pnrfornrps="";
			FlightSearch.ppspaymentmodesingle="";
			...
			if (EmerCont.equalsIgnoreCase("yes")) {
				...
				driver.findElement(By.xpath("//input[@name='Email Address']")).sendKeys("copa@auto.test");
...
*/			if (FlightSearch.getTrimTdata(queryObjects.getTestData("SplitPNR")).equalsIgnoreCase("yes")) {
				...
				if(FlightSearch.getTrimTdata(queryObjects.getTestData("Share_PNR")).equalsIgnoreCase("YES")) {
					oldPnr = ISharesflow.iPNRNo;					
				}
				else {
					oldPnr =Login.PNRNUM.trim();
				}
			...
			if (Reissue.equalsIgnoreCase("yes") && retval==true) {
				...
				if (queryObjects.getTestData("addSSRAfterPay").equalsIgnoreCase("yes")) {
					...
					if (driver.findElement(By.xpath("//button[contains(text(),'Check Out')]")).isEnabled()) {
						...
						BankNames = queryObjects.getTestData("MultipleFOPBankName_SSR").trim();
						InstallmentNums = queryObjects.getTestData("MultipleFOPInstallmentNum_SSR").trim();
						sr.MulFOP(driver,queryObjects,Double.parseDouble(sr.ssrAmount),FlightSearch.MultipleFOPType,FlightSearch.MultFOPsubType,FlightSearch.fopCardNums,BankNames,InstallmentNums,"SSRCASE");  //calling for multiple FOP....
						//Click on pay button
...
*/		}
		...
		if(pnrNum.equals("") || pnrNum.contains("share"))
			if(!emdNum.equalsIgnoreCase(""))//Modified by Navira
				...
				queryObjects.logStatus(driver, Status.WARNING, "PNR was not exist", "Test data issue" , null);
		else
			contflag=true;	
		if(!emdNum.isEmpty()){
			//Navira 15May
			if(emdNum.equalsIgnoreCase("RESEMD")) {
				if(Login.EMDNO.contains(";")){
					String emd[] = Login.EMDNO.split(";");
					emdNum = emd[0];//Taking the first EMD
				}
				else
					emdNum = Login.EMDNO;
			}
			else if(!Login.EMDNO.isEmpty()){
	...
	public static void reissue(WebDriver driver, BFrameworkQueryObjects queryObjects) throws Exception{
		...
		if(changeClassonly.equalsIgnoreCase("yes") && !changeRoute.equalsIgnoreCase("yes")) {
			...
			String Change_Class_Only=FlightSearch.getTrimTdata(queryObjects.getTestData("ChangeClassOnly"));
			if (Change_Class_Only.equalsIgnoreCase("yes")); 
				return;
		}
		...
		if(changeRoute.equalsIgnoreCase("yes")) {
			...
			if(changeDate.equalsIgnoreCase("yes")) {
				if (!sgetDate.isEmpty()) 
					...
					cal2.add(Calendar.DATE, Integer.parseInt(sgetDate));
				else
		...
		if (classselectbon==true){
			...
			String SYNCStatus = FlightSearch.getTrimTdata(queryObjects.getTestData("SYNCStatus"));
			...
			//Sync ticket yashodha
			if(SYNCStatus.equalsIgnoreCase("yes"))
			{
				//moved in sync ticket method by rushil
				SyncTicket(driver, queryObjects,Synchronize,RequestType);
				return;
			}
			
			String OutofSync = FlightSearch.getTrimTdata(queryObjects.getTestData("OutofSync"));
			
			if(OutofSync.equalsIgnoreCase("yes")) {
				return;
			}
			...
			/*	try {
				...
				if(FlightSearch.getTrimTdata(queryObjects.getTestData("AssEMD")).equalsIgnoreCase("yes")) {
					Thread.sleep(1000);
					driver.findElement(By.xpath("//div[@translate='pssgui.emd']")).click();
					FlightSearch.loadhandling(driver);
					Login.EMDNO=driver.findElement(By.xpath("(//div[contains(text(),'Baggage')])[2]/following::div[contains(@ng-click,'emdDetail')]")).getText();
					Thread.sleep(100);
	...
	public static boolean flightsearching(WebDriver driver,int viewnuberoflflightnoin,BFrameworkQueryObjects queryObjects) throws Exception{
		...
		String Specifitclass=FlightSearch.getTrimTdata(queryObjects.getTestData("classType_Specific"));
	...
	public static String Manual_reissue(WebDriver driver, BFrameworkQueryObjects queryObjects) throws Exception{
		...
		for (int Nopax=1;Nopax<=FlightSearch.totalnoofPAX;Nopax++){
			...
			if(FlightSearch.currencyType.trim().equalsIgnoreCase("ARS")|| FlightSearch.currencyType.trim().equalsIgnoreCase("COP") ){
				Fareamt=driver.findElement(By.xpath("//div[span[text()='Equivalent Fare']]/following-sibling::div[1]")).getText().trim();
				Thread.sleep(2000);
				driver.findElement(By.xpath("//input[@name='New Equivalent amount']")).sendKeys(Fareamt);  //equivalent fare net amount
				String FareamtCurr = driver.findElement(By.xpath("//div[span[text()='Equivalent Fare']]/following-sibling::div[3]")).getText().trim();
				Thread.sleep(2000);
			...
			if(!Add_Tax_Nm.isEmpty()){
				...
				if(!(taxnml.size()>0)) {  // it means if no tax available adding tax else skip
					...
					for (int Taxi = 0; Taxi < taxnml.size(); Taxi++) {
						...
						tax_Code=driver.findElements(By.xpath("//toggle-title[@class='ng-scope layout-align-space-between-center layout-row flex padding-left-5' and 1]/div[1]"));
			...
			String totlamt=String.valueOf(Double.parseDouble(Fareamt)+Taxamt);
			...
			if(FlightSearch.getTrimTdata(queryObjects.getTestData("EMD_History")).equalsIgnoreCase("yes")) {
				...
				if(driver.findElements(By.xpath("//div[@action='emd-history']//i[contains(@class,'icon-forward')]")).size()>0) {
					...
					if(date1.compareTo(date2) == 0) 
						queryObjects.logStatus(driver, Status.PASS, "Comparing the date of Issue on EMD Details and EMD History Page", "Issue Date is: "+date1+"in both Pages", null);
					else
						queryObjects.logStatus(driver, Status.FAIL, "Comparing the date of Issue on EMD Details and EMD History Page", "Issue Date is "+date1+" in Detals Page and "+date2+" in History Page", null);								
...
public static void SyncTicket(WebDriver driver, BFrameworkQueryObjects queryObjects, String Synchronize, String RequestType) throws Exception{
		...
		driver.findElement(By.xpath("//md-radio-button[@value='"+Synchronize+"']/child::div[1]")).click();
		Thread.sleep(1000);
		if(RequestType.equalsIgnoreCase("invol")) {
			driver.findElement(By.xpath("//md-radio-button[@value='true']/child::div[1]")).click();
		}else {
		...
		if(RequestType.equalsIgnoreCase("invol")) {
			String coupancntrl=driver.findElement(By.xpath("//td[contains(text(),'INVOL CM')]")).getText();
			
			if(coupancntrl.contains("INVOL CM"))
				queryObjects.logStatus(driver, Status.PASS, "SYnc", "successful" , null);
			else
				queryObjects.logStatus(driver, Status.FAIL, "SYnc", "Fail" , null);
				
		}else {
			String coupancntrl=driver.findElement(By.xpath("//span[contains(text(),'ADJUSTED')]")).getText();
