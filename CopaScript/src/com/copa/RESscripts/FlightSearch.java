...
public class FlightSearch extends FlightSearchPageObjects{
	...
	static String ppspaymentmodesingle="";//Getting Null pointer Exceptions
	static String ppspaymentmodedouble="";//Getting Null pointer Exceptions
	...
	public void PnrCreation(WebDriver driver, BFrameworkQueryObjects queryObjects) throws IOException{
		try{
			...
			Listssrname.add("SVAN");Listssrname.add("WEAP");Listssrname.add("FQTS");
			...
			if (Roundtrip.toUpperCase().equals("YES")){
				...
				if (Duration!="" && !Duration.equalsIgnoreCase("Same")) {
			...
			if (Noseg>0){
				...
				for (iter = iterNoseg; iter <= totlsegments; iter++) {
					...
					//Updated by Jenny - Oct 2019
					if (Duration!="" && Duration.equalsIgnoreCase("Same")) {
						calcseg.add(Calendar.DATE, (getDays));
					}else {
				...
				for (int iviewsiter = 0; iviewsiter <nuberofviews; iviewsiter++) {
					...
					if (sFlighNos!="" || (!envFlight.isEmpty())){
						...
						if (sFlighNos.contains(";")) {
							...
							if (arrFlighNos.length>nuberofTrips) {
								if (FlightCnt>1) {
									flightno=arrFlighNos[(tripiter+(FlightCnt-1))];
								}
							}
						}
						...
						if (getFlightno.equalsIgnoreCase(flightno)) {
							getflight=true;
							queryObjects.logStatus(driver, Status.INFO, "Searching for specific Flight", "specific flight was evailable", null);
							...
							FlightCnt = viewnuberoflflightnoin;
						...
						if (Duration.equalsIgnoreCase("Same") && !DestnTime.isEmpty()) {
						...
						if (SegTime) {
							...
							//----------
							if (sgetflighno.length()<6) {  // this is for avoid core chare flights need to check
								java.util.List<WebElement> lselectedflightno=driver.findElements(By.xpath(lselectedflightnoXpath));
								int viewnuberoflflightnoin;
								viewnuberoflflightnoin=lselectedflightno.size();
			...
			if(classselectbon==true) {
				...
				if(getTrimTdata(queryObjects.getTestData("Quote")).equalsIgnoreCase("yes")) {
					...
					if (quotescreemsg.contains("NO VALID FARE")|| quotescreemsg.contains("No fare is available")|| quotescreemsg.contains("No Price information") || quotescreemsg.contains("NO FARE FOR PASSENGER TYPE REQUESTED")){
				...
				//	gender = "Male";
					...
					if(!detailsEntered){
						...
						loadhandling(driver);
						//Thread.sleep(2000);
						//driver.findElement(By.xpath(nationalityXpath)).sendKeys(Keys.TAB);//Tab Alternate Solution
						try {
							...
							try {
								...
								try {
									driver.findElement(By.xpath("//md-virtual-repeat-container[contains(@aria-hidden,'false')]//span[text()='"+nation+"']/ancestor::span[2]/following-sibling::span/span/../../..")).click();
									}catch(Exception e3) {
										driver.findElement(By.xpath(nationalityXpath)).sendKeys(Keys.TAB);
									}
							}
						}
						//driver.findElement(By.xpath("//span[contains(text(),'" + nation + "')]")).click();
						...
						Thread.sleep(1000);
						loadhandling(driver);
						//driver.findElement(By.xpath(countrycodeXpath)).sendKeys(Keys.TAB);//Tab Alternate Solution
						try {
							driver.findElement(By.xpath(Login.SelList)).click();
						} catch (Exception e1) {							
							try {
								...
								try {
									driver.findElement(By.xpath("//md-virtual-repeat-container[contains(@aria-hidden,'false')]//span[text()='"+countryCode+"']/ancestor::span[2]/following-sibling::span/span/../../..")).click();
									}catch(Exception e3) {
						...
						driver.findElement(By.xpath(countrycodeselectXpath)).sendKeys("US");
						Thread.sleep(1000);
						loadhandling(driver);
						//driver.findElement(By.xpath(countrycodeselectXpath)).sendKeys(Keys.TAB);//Tab Alternate Solution
						try {
							driver.findElement(By.xpath(Login.SelList)).click();
						} catch (Exception e1) {							
							try {
					...
					//	driver.findElement(By.xpath("//button[contains(text(),'Check Out')]")).click();
						...
						if (pnrcreated && (!PymtError_Msg))
						{
							...
							if (driver.findElements(By.xpath("//span[contains(text(),'Open')]")).size()>0 && getTrimTdata(queryObjects.getTestData("Void")).equalsIgnoreCase("Ticket"))
							{
								...
								driver.findElement(By.xpath("//div[contains(@class,'md-active')]//div[contains(text(),'Void Ticket')]")).click();//Xpath Update
							...
							else if(errormsg.contains("SSR NOT AVAILABLE")){
								
								queryObjects.logStatus(driver, Status.WARNING, "After payment  Total amount checking", "After payment Totlal amount should show correctly actual: "+aact+" expected: "+aexpt, null);
								queryObjects.logStatus(driver, Status.WARNING, "After payment  Total amount checking","fare Amount is:"+Double.parseDouble(fareAmount) +" emd Amount is: "+emdAmount ,null);
							}
							else {
	...
	//Atul- 19 Mar to Click the authorize payment button
				Thread.sleep(2000);
				if (driver.findElement(By.xpath("//button[contains(text(),'Authorize Payment')]")).isDisplayed()) 
				driver.findElement(By.xpath("//button[contains(text(),'Authorize Payment')]")).click();
				FlightSearch.loadhandling(driver);
			...
			if (driver.findElement(By.xpath(activediv+"//input[@name='country']")).isDisplayed()) {
				...
				try {
					...
					try {
						...
						try {
							driver.findElement(By.xpath("//md-virtual-repeat-container[contains(@aria-hidden,'false')]//span[text()='"+cardholdecountry+"']/ancestor::span[2]/following-sibling::span/span/../../..")).click();
							}catch(Exception e3) {
								driver.findElement(By.xpath(activediv+"//input[@name='country']")).sendKeys(Keys.TAB);
							}
					}
	...
	public static void emailhandling(WebDriver driver,BFrameworkQueryObjects queryObjects) throws InterruptedException, IOException{
		loadhandling(driver);
		//Handling Email recipients popup
		...
		try{ 
							...
							if(paymentSeat.equalsIgnoreCase("yes")){
								...
								if (!assignSpecSeat.equals("")) {
									...
									if(assignSpecSeat.equalsIgnoreCase("exit")) {
										driver.findElement(By.xpath("//div[@aria-hidden='false']//md-option[div[contains(text(),'Exit')]]")).click();
		                                    ...
		                                    if(row.contains(";")) {//Multiple segments
			                                   ...
			                                   //for(int iter=0;iter<rowno.length;iter++) {
			                                     ...
			                                     if(dAmt==kk) {
			                                            ...
			                                            queryObjects.logStatus(driver, Status.WARNING, "Verifying the amount for payment seats", "Expected Amount: "+dAmt+" and Actual Amount: "+kk+" are not matching", null);            
		                                    }
		                                    else {
		                                           ...
		                                           if(dAmount==kk) {
		                                                  ...
		                                                  queryObjects.logStatus(driver, Status.FAIL, "Verifying the amount for payment seats", "Expected Amount: "+dAmount+" and Actual Amount: "+kk+" are matching", null);
			...
			if (getTrimTdata(queryObjects.getTestData("Expected_erromsg")).equalsIgnoreCase("") && (!paymentSeat.equalsIgnoreCase("yes"))){
				if(isSeatAssigned ) 
					...
					queryObjects.logStatus(driver, Status.FAIL, "Assigning Seat", "Seat should assign", null);
			}
		}
				...
				//	String Aftherpayemdval=driver.findElement(By.xpath("//td[contains(text(),'"+Login.envRead(Login.CURRENCY)+"')]")).getText().trim().split("\\s+")[0];
					...
					if(!EMDNum.isEmpty())
						...
						if (Atoflow.AddDateStr(0, "MMM", "day", null).toLowerCase().contains(gDte.substring(0, 3).toLowerCase()) && Atoflow.AddDateStr(0, "yyyy", "day", null).contains(gDte.substring(gDte.length()-4, gDte.length())) && gDay.contains(Atoflow.AddDateStr(0, "dd", "day", null))) {
							...
							queryObjects.logStatus(driver, Status.INFO, "EMD details - Verify Emd Creation date", "Emd creation date verification is successful " , null);
						}
						driver.findElement(By.xpath("//button[contains(text(),'Confirm')]")).click();
						emditer=emditer+1;
		...
		try{
			...
			if (driver.findElement(By.xpath("//div[contains(text(),'FOID Details')]")).isDisplayed()) {
				...
				if(driver.findElements(By.xpath("//button[text()='Next']")).size()==0) {
						driver.findElement(By.xpath("//button[text()='Pay' and contains(@aria-label,'print')]")).click();
						FlightSearch.loadhandling(driver);
						Thread.sleep(2000);
					}
					else {
	...
	public  void addSSR(WebDriver driver, BFrameworkQueryObjects queryObjects,String afterSplitssr,String addSSRSpecificSegment,String totSSRs,String ssrNames,String After_Pay_addSSR_OR_Book_Case_addSSR) throws Exception {
		...
		try {
			...
			for(int issr = 1;issr<=Integer.parseInt(totSSRs); issr++) {
				//Get Balance due to before adding SSR
				//if(issr==1){
				if(ssrcase.equalsIgnoreCase("yes") && (issr==1))
					balDueBeforeAddSSR=0;
				else {
					try {
						if (driver.findElement(By.xpath("//div[contains(@model,'payment.balanceDue')]")).isDisplayed()) {
							balDueBeforeAddSSR = Double.parseDouble(driver.findElement(By.xpath("//div[contains(@model,'payment.balanceDue')]")).getText().trim());
						}
					} catch (Exception e) {
						balDueBeforeAddSSR = 0;
	...
	public static Double selectingSSR(WebDriver driver, String ssrCode,BFrameworkQueryObjects queryObjects) throws InterruptedException, IOException {
		...
		else if(ssrCode.matches("VGML")) {//Enter Explanation
			ssrAmt=ssramtget(driver, queryObjects, EMDA_SSSRAmtXpath);  //amount calculation
		}
		else if(ssrCode.matches("KSML")) {//Enter Explanation
			driver.findElement(By.xpath("//md-content[contains(@ng-if,'ssrSubcategory.model.Passengers')]/div[1]//input")).click();
		//	driver.findElement(By.xpath("//md-content[contains(@ng-if,'ssrSubcategory.model.Passengers')]/div[1]//input")).sendKeys("2s");
			Thread.sleep(1000);
		//Copy to all legs
		//	driver.findElement(By.xpath("//md-content[contains(@ng-if,'ssrSubcategory.model.Passengers')]/div[1]//span[contains(@ng-click,'copyToLegs')]")).click();
		...
		else if(ssrCode.matches("FPML")) {//Enter Explanation
			driver.findElement(By.xpath("//md-content[contains(@ng-if,'ssrSubcategory.model.Passengers')]/div[1]//input")).click();
			driver.findElement(By.xpath("//md-content[contains(@ng-if,'ssrSubcategory.model.Passengers')]/div[1]//input")).sendKeys("2s");
			Thread.sleep(1000);
			//Copy to all legs
			...
			ssrAmt=ssramtget(driver, queryObjects, EMDA_SSSRAmtXpath);  //amount calculation   
		} else if(ssrCode.matches("BIKE")) {//Enter Explanation
			driver.findElement(By.xpath("//md-content[contains(@ng-if,'ssrSubcategory.model.Passengers')]/div[1]//input")).click();
		...
		//	driver.findElement(By.xpath("//md-content[contains(@ng-if,'ssrSubcategory.model.Passengers')]/div[1]//span[contains(@ng-click,'copyToLegs')]")).click();
			...
			ssrAmt=ssramtget(driver, queryObjects, EMDA_SSSRAmtXpath);  //amount calculation   
		} else if(ssrCode.matches("SEMN")) {//Enter Explanation
			driver.findElement(By.xpath("//md-content[contains(@ng-if,'ssrSubcategory.model.Passengers')]/div[1]//input")).click();
			driver.findElement(By.xpath("//md-content[contains(@ng-if,'ssrSubcategory.model.Passengers')]/div[1]//input")).sendKeys("2s");
			Thread.sleep(1000);
			//Copy to all legs
			driver.findElement(By.xpath("//md-content[contains(@ng-if,'ssrSubcategory.model.Passengers')]/div[1]//span[contains(@ng-click,'copyToLegs')]")).click();
			ssrAmt=ssramtget(driver, queryObjects, EMDA_SSSRAmtXpath);  //amount calculation   
		} else if(ssrCode.matches("GFML")) {//Enter Explanation
		 ...
		 else if(ssrCode.matches("AOXY")) {//Enter Explanation
				driver.findElement(By.xpath("//md-content[contains(@ng-if,'ssrSubcategory.model.Passengers')]/div[1]//input")).click();
				driver.findElement(By.xpath("//md-content[contains(@ng-if,'ssrSubcategory.model.Passengers')]/div[1]//input")).sendKeys("2s");
				Thread.sleep(1000);
				//Copy to all legs
			...
			ssrAmt=ssramtget(driver, queryObjects, EMDA_SSSRAmtXpath);  //amount calculation   
		} else if(ssrCode.matches("2NDB")) {//Enter Explanation
			driver.findElement(By.xpath("//md-content[contains(@ng-if,'ssrSubcategory.model.Passengers')]/div[1]//input")).click();
			driver.findElement(By.xpath("//md-content[contains(@ng-if,'ssrSubcategory.model.Passengers')]/div[1]//input")).sendKeys("2s");
			Thread.sleep(1000);
			//Copy to all legs
			...
			ssrAmt=ssramtget(driver, queryObjects, EMDA_SSSRAmtXpath);  //amount calculation   
		} else if(ssrCode.matches("DBU1 FLYUP BIDDING UP")) {//Enter Explanation
			driver.findElement(By.xpath("//md-content[contains(@ng-if,'ssrSubcategory.model.Passengers')]/div[1]//input")).click();
			driver.findElement(By.xpath("//md-content[contains(@ng-if,'ssrSubcategory.model.Passengers')]/div[1]//input")).sendKeys("2s");
			Thread.sleep(1000);
			//Copy to all legs
			...
			ssrAmt=ssramtget(driver, queryObjects, EMDA_SSSRAmtXpath);  //amount calculation   
		} else if(ssrCode.matches("DBU3 FLYUP  INSTANT")) {//Enter Explanation
			driver.findElement(By.xpath("//md-content[contains(@ng-if,'ssrSubcategory.model.Passengers')]/div[1]//input")).click();
			driver.findElement(By.xpath("//md-content[contains(@ng-if,'ssrSubcategory.model.Passengers')]/div[1]//input")).sendKeys("2s");
			Thread.sleep(1000);
			//Copy to all legs
			...
			//Enter Explanation
			driver.findElement(By.xpath("//md-content[contains(@ng-if,'ssrSubcategory.model.Passengers')]/div[1]//input")).click();
			driver.findElement(By.xpath("//md-content[contains(@ng-if,'ssrSubcategory.model.Passengers')]/div[1]//input")).sendKeys("1cs");
			Thread.sleep(1000);
			//Copy to all legs
			driver.findElement(By.xpath("//md-content[contains(@ng-if,'ssrSubcategory.model.Passengers')]/div[1]//span[contains(@ng-click,'copyToLegs')]")).click();
			ssrAmt=ssramtget(driver, queryObjects, EMDA_SSSRAmtXpath);  //amount calculation
	...
	public void PAXreductionType(WebDriver driver,BFrameworkQueryObjects queryObjects,String paxReductionTypeCount,String paxReductionTypeName) throws Exception{
		...
		if(Integer.parseInt(paxReductionTypeCount) == 1) {
			...
			Thread.sleep(1000);
			driver.findElement(By.xpath(PaxReductiontypeXpath)).sendKeys(Keys.TAB);
	...
	public void AgencyPayment(WebDriver driver,BFrameworkQueryObjects queryObjects) throws IOException {
		try{
			...
			loadhandling(driver);
			Thread.sleep(4000);//Wait required - Himani
			// Navira 
	...
	public static double Manual_Quote(WebDriver driver,BFrameworkQueryObjects queryObjects,String worningCase,String ManulaQuote_Fare_amt,String ManualQuote_Euivalent_amt,Double Service_Fee) throws InterruptedException, IOException{
		...
		for (int iquote = 0; iquote < Manual_Quote.size(); iquote++) {
			...
			if(worningCase.equalsIgnoreCase("YES"))	{   
				...
				fareAmountManualQuote=fareAmountManualQuote+Double.parseDouble(EuivalenAmt);
			}
			else{
				//String amt="2000";
...
public static void waivermethod(WebDriver driver, BFrameworkQueryObjects queryObjects, String wave, String Waiver_Fee, String withoutPnr_paxnm) throws Exception{
	...
	public void Discount(WebDriver driver, BFrameworkQueryObjects queryObjects,Boolean BeforeCreatePNR,String discountType,String dispax, String segments,String discValue,String valueType,String discTax,String ticketDesignator ) throws NumberFormatException, IOException, InterruptedException, Exception{
		//NS
			...
			if(valueType.contains("AMOUNT"))
				driver.findElement(By.xpath("//div[contains(text(),'Amount')]")).click();
			else if(valueType.contains("PERCENTAGE"))
			...
			if(discTax.contains("AFTER"))
				driver.findElement(By.xpath("//md-option[@value='PostTax']//div[contains(text(),'Taxes')]")).click();
			else if(discTax.contains("BEFORE"))
				driver.findElement(By.xpath("//md-option[@value='PreTax']//div[contains(text(),'Taxes')]")).click();
			Thread.sleep(2000);
	...
	//Start - Navira
		...
		public void ARNK(WebDriver driver, BFrameworkQueryObjects queryObjects) throws IOException, Exception{
