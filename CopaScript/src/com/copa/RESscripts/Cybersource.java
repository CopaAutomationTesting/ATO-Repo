...
public class Cybersource {
	...
	public static String Password = "Jan.Feb-2020";
					...
					if(FlightSearch.Aftecybebersourec_Update_check_GUI){   // this is for create PNR and reissue case After update cyber sourece checking in pss GUI ticket status.....
						if(Accept_check_CCSGUI){
							if(driver.findElements(By.xpath("//div[@translate='Ticketed']")).size()>0)
								...
								queryObjects.logStatus(driver, Status.FAIL, "After update  CyberSource Ticket Status checking", "cybersourec Accept case  Ticket status should displaying correctly like Ticketed" , null);
						}
							...
							if(driver.findElements(By.xpath("//div[@translate='Not Ticketed']")).size()>0)
								...
								queryObjects.logStatus(driver, Status.FAIL, "After update  CyberSource Ticket Status checking", "cybersourec Reject case  Ticket status should displaying correctly like Non Ticketed" , null);
						}
	...
	else 
	{
		...
		//Perform Reject , Accept actions on ticket
			if (Biller_Email.contains("review")) {
				...
				//checking the code  for Accept -----481
				...
				if (driver.findElement(By.xpath("//div[contains(text(),'Order Rejected')]")).isDisplayed()) {
		...
		queryObjects.logStatus(driver, Status.FAIL, "VerifyBillerDetails----->", "Fail", null);
	}
		}
	...
	private static void PerformActiononTickets(WebDriver driver, BFrameworkQueryObjects queryObjects, String Tkt_Action) throws InterruptedException, IOException {
		...
		if(Tkt_Action.contains("Accept")) {
			...
			if (driver.findElement(By.xpath("//p[contains(text(),'The order was accepted successfully.')]")).isDisplayed()) {
				queryObjects.logStatus(driver, Status.PASS, "Reject the order", "The order was accepted successfully is displayed",null);
				//kis
