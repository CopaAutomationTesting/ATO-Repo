...
public class PNRsearch extends FlightSearchPageObjects{
	...
	public static List<String> etktsnum;
		...
		try {
			...
			if(FlightSearch.getTrimTdata(queryObjects.getTestData("AddInfant")).equalsIgnoreCase("yes")) {
				...
				driver.findElement(By.xpath(datepickerXpath)).sendKeys(timeStamp);
					...
					BankNames = queryObjects.getTestData("MultipleFOPBankName_Quote").trim();
					InstallmentNums = queryObjects.getTestData("MultipleFOPInstallmentNum_Quote").trim();
			...
			if (contflag && srtReissue.equalsIgnoreCase("yes")) {
				...
				BankNames = queryObjects.getTestData("MultipleFOPBankName").trim();
				InstallmentNums = queryObjects.getTestData("MultipleFOPInstallmentNum").trim();
...
//					FlightSearch.loadhandling(driver);	//indrajit
			...
			String EmdTypcheck=FlightSearch.getTrimTdata(queryObjects.getTestData("EmdTypcheck"));
			if(EmdTypcheck.equalsIgnoreCase("yes")) {
				Thread.sleep(1000);
				String ErrorMsg=FlightSearch.getErrorMSGfromAppliction(driver, queryObjects);
				if(ErrorMsg.contains("Disassociated EMD(s) exist")) 
					queryObjects.logStatus(driver, Status.PASS, "Disassociated EMD","Exist" , null);
				else 
					queryObjects.logStatus(driver, Status.WARNING, "Disassociated EMD","Doesnt exist" , null);
	...
	public static int EMDticketcount(WebDriver driver, BFrameworkQueryObjects queryObjects) throws Exception{
	...
	public static String SplitBooking(WebDriver driver, BFrameworkQueryObjects queryObjects , String pnrNum, String cSelection)throws Exception{
		String newpnrpop = "";
		try {
		...
		if (!cSelection.isEmpty()) {
			driver.findElement(By.xpath("//md-dialog//div[div[@class='ng-scope']]/div["+cSelection+"]/md-checkbox")).click();
		} else {
		...
		newpnrpop=driver.findElement(By.xpath("//div[@action='pnr']/div/div[1]/div[1]/div[1]")).getText().trim();
		SplitPNR = newpnrpop; //Atul- 7Apr
		...
		if(afterSplitssr.equalsIgnoreCase("yes"))
		{
			...
			String sssrcaseBalanceamt=FlightSearch.getTrimTdata(driver.findElement(By.xpath("//div[@model='shoppingCart.model.payment.balanceDue']/div")).getText());
	...
	public static void reQuote(WebDriver driver, BFrameworkQueryObjects queryObjects, String WaivePax, String WaiveFee, String QuoteSeg, String SelPax) throws Exception{
		...
		String Psgtype=FlightSearch.getTrimTdata(queryObjects.getTestData("Psgtype"));
		if(!TourCode.equalsIgnoreCase("")){
			if(Psgtype.equalsIgnoreCase("nrps")||Psgtype.equalsIgnoreCase("nrsa"))
			{
				driver.findElement(By.xpath("//md-select[contains(@aria-label,'account Codes')]")).click();
				Thread.sleep(1000);
				driver.findElement(By.xpath("//md-option[@value='"+TourCode+"']")).click();
			}
			else {
       ...
       // Login.envwrite(Login.shareflightnm, FirstflightforAto);
		...
		if (quotescreemsg.contains("NO VALID FARE")|| quotescreemsg.contains("No fare is available")){
...
public static void Advance_Search(WebDriver driver, BFrameworkQueryObjects queryObjects) throws Exception{
		...
		try {
			...
			try {
				...
				try {
					...
					try {
						driver.findElement(By.xpath("//md-virtual-repeat-container[contains(@aria-hidden,'false')]//span[text()='"+Adv_countryCode+"']/ancestor::span[2]/following-sibling::span/span/../../..")).click();
						}catch(Exception e3) {
							driver.findElement(By.xpath("//input[@name='countryCode']")).sendKeys(Keys.TAB);
						}
				}
			...
			try {
				...
				try {
					...
					try {
						...
						try {
							driver.findElement(By.xpath("//md-virtual-repeat-container[contains(@aria-hidden,'false')]//span[text()='"+Adv_stateCode+"']/ancestor::span[2]/following-sibling::span/span/../../..")).click();
							}catch(Exception e3) {
	...
	public static boolean Search_PNR(WebDriver driver, BFrameworkQueryObjects queryObjects) throws Exception{
		Thread.sleep(2000);
		driver.findElement(By.xpath("//md-menu[@ng-model='layout.pssguiModules.module']/button")).click();  // clicking reservation menu bar
		 Thread.sleep(2000);
		 driver.findElement(By.xpath("//div[contains(@class,'_md md-open-menu-container') and contains(@class,'md-active')]//button[contains(text(),'Reservations')]")).click();
