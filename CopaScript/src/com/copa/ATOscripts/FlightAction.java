	...
	public void FlightActions(WebDriver driver, BFrameworkQueryObjects queryObjects) throws Exception {
		...
		if(MassTransfer.equalsIgnoreCase("Y")){
			try{
				...
				try{
					if(driver.findElement(By.xpath("//div[contains(text(),'Mass Transfer') and contains(text(),'"+TransferOption+"')]")).isDisplayed()){
				...
				try{
					...
					if (driver.findElements(By.xpath("//div[contains(text(), 'IRROP-DELAY') and contains(text(),'"+Reason+"')]")).size()>0) {
						queryObjects.logStatus(driver, Status.PASS, "Automatic OSI in PNR ", "OSI"+Reason+"is displayed in reservation of the passenger affected by mass transfer ", null);		
					} else {
